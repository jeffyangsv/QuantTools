...
if __name__ == '__main__':
    ...
    if QA_util_if_trade(QA_util_today_str()):
        check_day ='2022-07-17'
        htmlbody=reademail('目标股池{}'.format(check_day))
        data1 = pd.read_html(htmlbody[0], encoding='gbk', header=0)[0]
        data1=data1[data1.RANK.notna()].rename(columns={'Unnamed: 0':'date','Unnamed: 1':'code'})
        data1=data1[data1.date == QA_util_get_real_date(check_day)]
        htmlbody=reademail('综合选股报告 一{}'.format(check_day))
        data2 = pd.read_html(htmlbody[0], encoding='gbk', header=0)[0]
        data2=data2[data2.RANK.notna()].rename(columns={'Unnamed: 0':'date','Unnamed: 1':'code'})
        data2=data2[data2.date == QA_util_get_real_date(check_day)]
