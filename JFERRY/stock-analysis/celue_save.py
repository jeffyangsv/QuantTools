...
tdxhy_drop = ["T110201","T110202","T110204"]  # list类型。记事本打开 通达信目录\incon.dat，查看#TDXNHY标签的行业代码。T1002=证券


def celue_save(file_list, HS300_signal, tqdm_position=None):
    ...
    for stockcode in tq:
        ...
        if True in df['celue_buy'].isna().to_list():
            ...
            end_date = df.index[-1]
            celue2 = CeLue.Strategy2(df, HS300_signal, start_date=start_date, end_date=end_date)
            celue_sell = CeLue.SellStrategy(df, celue2, start_date=start_date, end_date=end_date)
...
if __name__ == '__main__':
    ...
    HS300_signal = CeLue.signalHS300(df_hs300)
    ...
    if 'single' in sys.argv[1:]:
        ...
        df_celue = celue_save(stocklist, HS300_signal)
    else:
        # 多线程。好像没啥效果提升
        # threads = []
        # t_num = 4  # 线程数
        ...
        #         threads.append(threading.Thread(target=celue_save, args=(stocklist[i*div:(i+1)*div], HS300_signal)))
        #     else:
        #         # print(i, i * div, (i + 1) * div + mod)
        #         threads.append(threading.Thread(target=celue_save, args=(stocklist[i*div:(i+1)*div+mod], HS300_signal)))
        # # celue_save(stocklist, HS300_signal)
        #
        # print(threads)
        # for t in threads:
        #     t.setDaemon(True)
        ...
        for i in range(0, t_num):
            ...
            if i + 1 != t_num:
                # print(i, i * div, (i + 1) * div)
                pool_result.append(p.apply_async(celue_save, args=(stocklist[i * div:(i + 1) * div], HS300_signal, i)))
            else:
                # print(i, i * div, (i + 1) * div + mod)
                pool_result.append(
                    p.apply_async(celue_save, args=(stocklist[i * div:(i + 1) * div + mod], HS300_signal, i)))
        # celue_save(stocklist, HS300_signal)
