...
def signalHS300(df_hs300, start_date='', end_date=''):
    """
    ...
    HS300_drate = (HS300_CLOSE / REF(HS300_CLOSE, 1) - 1) * 100     #当日涨幅
    HS300_signal = ~(HS300_drate < -1.5) & ~(HS300_drate > 1.5)      #跌幅不大于1.5 且涨幅不大于1.5
    return HS300_signal


def Strategy1(df, start_date='', end_date='', mode=None):
    """
...
def Strategy2(df, HS300_signal, start_date='', end_date=''):
    """
    ...
    # TJ11
    TJP1 = Strategy1(df, start_date, end_date)
...
def SellStrategy(df, Strategy2, start_date='', end_date=''):
    """

    :param df: 个股Dataframe
    :param Strategy2: 买入策略2
    :param start_date:
    :param end_date:
    :return: 卖出策略序列
    """

    if True not in Strategy2.to_list():  # 买入策略2 没有买入点
        return pd.Series(index=Strategy2.index, dtype=bool)
...
if __name__ == '__main__':
    ...
    HS300_signal = signalHS300(df_hs300)
    ...
    celue1_fast = Strategy1(df_stock, mode='fast', start_date=start_date, end_date=end_date)
    celue1 = Strategy1(df_stock, mode='', start_date=start_date, end_date=end_date)
    celue2 = Strategy2(df_stock, HS300_signal, start_date=start_date, end_date=end_date)
    celue_sell = SellStrategy(df_stock, celue2, start_date=start_date, end_date=end_date)
