...
def QA_etl_stock_info(ui_log= None):
    ...
    data = QA_fetch_stock_industryinfo(codes).reset_index(drop=True)
    ...
    if type == 'all' and len(codes) > 0:
        for code in codes:
            ...
            if data is None:
                ...
                QA_util_sql_store_mysql(data[columns].reset_index(drop=True).fillna(0), "stock_financial",if_exists='append')
                QA_util_log_info(
...
def QA_etl_stock_financial_day(start_date = QA_util_today_str(), end_date= None,ui_log= None):
    ...
    data = QA_fetch_stock_fianacial_adv(codes, start_date, end_date).data[[ 'INDUSTRY','TOTAL_MARKET', 'TRA_RATE', 'DAYS',
                                                                            ...
                                                                            'PE_RATE','PEEGL_RATE','PB_RATE','ROE_RATE','ROA_RATE',
                                                                            ...
                                                                            'ROE', 'ROE_TTM',#'ROE_L2Y', 'ROE_L3Y', 'ROE_L4Y', 'ROE_LY',
...
def QA_etl_stock_technical_15min(start_date = QA_util_today_str(), end_date= None, ui_log= None):
    if end_date is None:
        end_date = QA_util_today_str()
    QA_util_log_info('##JOB Now ETL STOCK TECHNICAL 15min ==== from {from_} to {to_}'.format(from_=start_date,to_=end_date), ui_log)
    codes = list(QA_fetch_stock_all()['code'])
...
def QA_etl_index_technical_30min(start_date = QA_util_today_str(), end_date= None, ui_log= None):
    if end_date is None:
        end_date = QA_util_today_str()
    QA_util_log_info('##JOB Now ETL INDEX TECHNICAL 30min ==== from {from_} to {to_}'.format(from_=start_date,to_=end_date), ui_log)
    codes = list(QA_fetch_index_list_adv().code)
    data = QA_fetch_index_technical_index_adv(codes, start_date, end_date,type='30min').data
    if data is None:
        QA_util_log_info(
            '##JOB NO INDEX TECHNICAL 30min HAS BEEN SAVED ==== from {from_} to {to_}'.format(from_=start_date,to_=end_date), ui_log)
    else:
        data = data.reset_index()
        data = data.assign(date=data.date.apply(lambda x:datetime.datetime.strptime(x,'%Y-%m-%d')))
        QA_util_sql_store_mysql(data, "index_technical_30min",if_exists='append')
